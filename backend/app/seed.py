...
async def seed_work_types(db: AsyncSession):
    for name in ["测量员", "绘图员", "技术员", "项目经理", "项目负责", "内业"]:
...
async def seed_users(db: AsyncSession):
    # 管理员
    admin_emp = Employee(employee_code="EMP2025001", name="系统管理员", work_type="技术员",
                         personnel_type="事业人员", department="地理信息院", position="系统管理员",
                         daily_wage=488, status="在职")
    db.add(admin_emp); await db.flush()
    if not (await db.execute(select(User).where(User.username == "admin"))).scalar_one_or_none():
        db.add(User(username="admin", password_hash=hash_pw(seed_password("SEED_ADMIN_PASSWORD", "admin123")), role="director", employee_id=admin_emp.id))

    # 项目经理 - 李赫
    lh_emp = Employee(employee_code="EMP2025002", name="李赫", work_type="项目经理",
                      personnel_type="事业人员", department="地理信息院", position="项目负责",
                      daily_wage=488, status="在职")
    db.add(lh_emp); await db.flush()
    if not (await db.execute(select(User).where(User.username == "lihe"))).scalar_one_or_none():
        db.add(User(username="lihe", password_hash=hash_pw(seed_password("SEED_LIHE_PASSWORD", "123456")), role="manager", employee_id=lh_emp.id))

    # 员工 - 宋英伦（事业人员）
    syl_emp = Employee(employee_code="EMP2025003", name="宋英伦", work_type="测量员",
    ...
    if not (await db.execute(select(User).where(User.username == "songyinglun"))).scalar_one_or_none():
        db.add(User(username="songyinglun", password_hash=hash_pw(seed_password("SEED_SONGYINGLUN_PASSWORD", "123456")), role="employee", employee_id=syl_emp.id))
    ...
    # 企业人员 - 测试
    ent_emp = Employee(employee_code="EMP2025005", name="企业员工1", work_type="测量员",
                       personnel_type="企业人员", department="地理信息院", position="测量员",
                       daily_wage=238, status="在职")
    db.add(ent_emp);
    await db.flush()
